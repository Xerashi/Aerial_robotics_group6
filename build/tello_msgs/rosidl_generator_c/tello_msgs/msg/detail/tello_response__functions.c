  ...
  tello_msgs__msg__TelloResponse__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tello_msgs__msg__TelloResponse * data =
      (tello_msgs__msg__TelloResponse *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tello_msgs__msg__TelloResponse__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tello_msgs__msg__TelloResponse__fini(&output->data[i]);
        }
        return false;
      }
    }
